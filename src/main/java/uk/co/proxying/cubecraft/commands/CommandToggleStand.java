 ...
 */
public class CommandToggleStand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        if (args.length == 1) {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                ...
                dataWatcher.register(new DataWatcherObject<>(0, DataWatcherRegistry.a), (byte) 0);
                packet = new PacketPlayOutEntityMetadata(player.getEntityId(), dataWatcher, true);
                targetNMS.playerConnection.sendPacket(packet);
            }

            commandSender.sendMessage(target.getDisplayName() + " should see everyone standing now.");
            return true;
        }
        return false;
    }
